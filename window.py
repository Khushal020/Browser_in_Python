...
class Window(QGraphicsScene):
    ...
    def __init__(self, view):
        ...
        self.mydb = mysql.connector.connect(host="host_name",user="user",password="password", database = 'database_name')
        
        self.addNewTabFirst()
        self.addNewTabSecond()
        self.addNewTab()

        button = QPushButton(QIcon('./res/plus.png'), '')
    ...
    def addNewTabFirst(self):

        qurl = QUrl('https://www.myopentip.com/')

        web = QWebView()
        web.load(qurl)

        i = self.tabs.addTab(web, "MyOpenTip")
    ...
    def addNewTabSecond(self):

        qurl = QUrl('https://www.bizxu.com/')

        web = QWebView()
        web.load(qurl)

        i = self.tabs.addTab(web, "Bizxu")
    ...
    def addToHistory(self, title, link):
        ...
        url = urlparse(link)
        print(link, url.netloc)
        try:
            recIP = socket.gethostbyname(url.netloc)
        except:
            recIP = ''
        myIP = get('https://api.ipify.org').text

        cursor = self.mydb.cursor()
        query = "insert into history values('" + str(title) + "', '" + str(myIP) + "', '" + str(recIP) + "', '" + str(link) + "', NOW());"
...
# History Button
    def historyButtonPush(self):
        ...
        # cursor = self.mydb.cursor()
        # rows = cursor.execute('select * from history')
        # result = cursor.fetchall()
        
        # for line in result:
        #     url = QUrl(line[3])
        #     self.list.addItem(QListWidgetItem(line[0] + ' |-| ' + line[1] + ' |-| ' + line[2] + ' |-| ' + line[3] + ' |-| ' + str(line[4])))
    ...
    def deleteAllHistory(self):
        self.history = []
        self.lines = []
        file = open('./res/history.txt', 'w+')
        lines = file.readlines()
        file.close()

        # cursor = self.mydb.cursor()
        # cursor.execute('delete from history where 1;')
        # self.mydb.commit()
...
# Bookmark Button
    ...
    def bookmarksButtonPush(self):
        ...
        file = open('./res/bookmarks.json', 'r')
    ...
    def deleteBookmark(self):
        ...
        file.close()
        file = open('./res/bookmarks.json', 'w+')
